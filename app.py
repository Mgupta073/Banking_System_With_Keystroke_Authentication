...
def client_regis_template(message=None):
    return render_template(
        "clientRegis.html", message=message if message is not None else None
    )
...
def redirect_home(message=None):
    return render_template(
        "index.html", message=message if message is not None else None
    )
...
def redirect_home_temp(message=None):
    ...
    print("hey hero i am here!!!")
    r1 = open(
        "/Users/krish.u/Desktop/KeyStroke/Banking-System-with-Keystroke-Authentication/user.csv"
    )
    r = csv.reader(r1)
    lines = list(r)
    lines[0][0] = tempvalue
    ...
    fout = open(
        "/Users/krish.u/Desktop/KeyStroke/Banking-System-with-Keystroke-Authentication/keystroke.trial.csv",
        "a",
    )
    for line in r1:
        fout.write(line)
    ...
    class SVMDetector:
        ...
        def evaluate(self):
            ...
            for subject in subjects:
                genuine_user_data = data.loc[
                    data.subject == tempvalue, "H.period":"UD.l.Return"
                ]
                ...
                self.test_imposter = (
                    imposter_data.groupby("subject")
                    .head(20)
                    .loc[:, "H.period":"UD.l.Return"]
                )

                self.training()
                self.testing()
                eers.append(evaluateEER(self.u_scores, self.i_scores))
                break
            return np.mean(eers)

    path = "/Users/krish.u/Desktop/KeyStroke/Banking-System-with-Keystroke-Authentication/keystroke.trial.csv"
    data = pd.read_csv(path)
    subjects = data["subject"].unique()
    print(subjects)
    print(SVMDetector(subjects).evaluate())
    if (
        SVMDetector(subjects).evaluate() < 0.01
        and SVMDetector(subjects).evaluate() > 0.0
    ):
        return render_template(
            "index.html", message=message if message is not None else None
        )
        ...
        return render_template(
            "clienthome.html", message=message if message is not None else None
        )
...
def redirect_reg_captcha(message=None):
    ...
    # Instead of assuming 20 rows exist, write to however many rows we have
    num_rows = len(lines)
    if num_rows == 0:
        # If file is empty, create a new row
        lines.append([tempvalue_reg])
    else:
        # Update existing rows, up to 20 or the number of rows we have
        max_rows = min(20, num_rows)
...
def redirect_about(message=None):
    return render_template(
        "about.html", message=message if message is not None else None
    )
...
def redirect_post(message=None):
    return render_template(
        "post.html", message=message if message is not None else None
    )
...
def redirect_contact(message=None):
    return render_template(
        "contact.html", message=message if message is not None else None
    )
...
def redirect_client_transaction(message=None):
    return render_template(
        "clienttransactions.html", message=message if message is not None else None
    )
...
def redirect_client_fundtransfer(message=None):
    if request.method == "GET":
        return render_template(
            "client_fundtransfer.html", message=message if message is not None else None
        )
    else:
        ...
        Client.transfer_fund(
            self_acc, recieving_acc, recieving_phone, int(Amount), Desc
        )
...
def redirect_client_logfiles(message=None):
    return render_template(
        "client_logfile.html", message=message if message is not None else None
    )
...
def admin_login_template(message=None):
    return render_template(
        "adminlogin.html", message=message if message is not None else None
    )
...
def user_login_template(message=None):
    return render_template(
        "userlogin.html", message=message if message is not None else None
    )
...
def login_client():
    if request.method == "POST":
        ...
        if client is not None and client.password == password:
            session["email"] = client.email
            return render_template("login_captcha.html", target="blank")

    # Handle captcha redirection from `/client_home`
    if session.get("email"):
        return render_template("login_captcha.html", target="blank")
    
    # If no session exists, return to home/login page
    session["email"] = None
    return render_template("home.html", target="blank")
...
def emp_add():
    if request.method == "GET" and session["email"] != None:
        ...
        client = Client(
            _id, password, name, Contact_number, Gender, email, Date_of_Birth
        )
...
def admin_manage_clients_template(message=None):
    return render_template(
        "manageclientdetails.html", message=message if message is not None else None
    )
...
def admin_delete_clients_template(message=None):
    return render_template(
        "delete-client.html", message=message if message is not None else None
    )
...
def admin_log_files_template(message=None):
    return render_template(
        "adminlogfile.html", message=message if message is not None else None
    )
...
def admin_home_template(message=None):
    return render_template(
        "home-admin.html", message=message if message is not None else None
    )
